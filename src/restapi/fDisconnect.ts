...
export const fDisconnect:CallResponseHandler = async (req, res) => {
    ...
    const config = await tryCallResponseWithMessage(configStore.getValues(), 'fDisconnect - Unable to get config store values', res);
    if (!config) {
        return;
    }
    ...
    const tokens: ZDTokensResponse = await tryCallResponseWithMessage(zdClient.oauthtokens.list(), 'fDisconnect - Unable to list zendesk oauth tokens', res);
    if (!tokens) {
        return;
    }
    ...
    try {
        ...
        const callResponse = newErrorCallResponseWithMessage('fDisconnect - failed to revoke acting user token');
        res.json(callResponse);
        return;
    }

    const callResponse = newOKCallResponseWithMarkdown('You have disconnected your Zendesk account');
    res.json(callResponse);
};
